...
ピラミッドの左半分だけについて考える。以後ピラミッドといったら左半分のことを指すことにする(頂上含む)。

$L_i$ を $i$ 番目の要素が頂上であるときのピラミッドの最大サイズとすると、
$L_i = \min(A_i, L_{i-1} + 1)$ である。

これを右半分のピラミッドについても同様に計算し、$R_i$ とする。

求める答えは次式で表される。

<!-- dprint-ignore -->
\begin{align}
    \max_{i} \left\\{ \min(L_i, R_i) \right\\}
\end{align}
